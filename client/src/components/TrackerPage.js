  ...
  return (
    <div className="app-container">
      ...
      <div className="habit-tracker">
        {isLoading ? (
          <div className="status-message">Loading habits... ⏳</div>
        ) : habits.length === 0 ? (
          <div className="status-message">No habits yet. Add one to get started! ✨</div>
        ) : (
          <>
            <div className="header-row">
              <div className="habit-name-header">Habit</div>
              <div className="date-headers-container"> {/* Naya container */}
                {weekDates.map((date) => (
                  <div key={date} className="date-header">{new Date(date).getDate()}</div>
                ))}
              </div>
            </div>
            <AnimatePresence>
                ...
                >
                  <div className="habit-name">
                    <span className="habit-name-text" onClick={() => viewHabitDetails(habit._id)}>
                      {habit.name}
                      {habit.streak > 0 && (
                        <span className="streak-counter">🔥 {habit.streak}</span>
                      )}
                    </span>
                    <button onClick={() => deleteHabit(habit._id)} className="delete-btn">🗑️</button>
                  </div>
                  {/* Status boxes ke liye bhi naya container */}
                  <div className="status-boxes-container">
                    {weekDates.map((date) => (
                      <motion.div 
                        key={date} 
                        className={`status-box ${getStatusForDate(habit, date)}`}
                        onClick={() => updateHabitStatus(habit._id, date, getStatusForDate(habit, date))}
