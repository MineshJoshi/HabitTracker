...
function TrackerPage({ token, viewHabitDetails }) { // Token ko props se liya
  ...
  // Yahan badlav kiya hai: [token] ko dependency me daala hai
  useEffect(() => {
    if (token) {
        fetchHabits();
    }
  ...
  const fetchHabits = async () => {
    ...
    try {
      ...
      setHabits(response.data);
        ...
        // Agar server so gaya hai, toh user ko batayein
